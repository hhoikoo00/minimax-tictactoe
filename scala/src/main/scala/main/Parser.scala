...
object Parser {
  ...
  def boardSize(): Int = {
    ...
    } { _ => print("Board size must be greater than 2 > ") }
  }
  ...
  /**
   * Parses user input until a valid maximum depth for depth-limit search inputted (l > 0)
   *
   * @return A valid maximum level for depth-limit search
   */
  def maxLevel(): Int = {
    print("""What is the maximum depth the AI should search for? (l > 0) > """)
    retryUntilSuccess {
      Try(StdIn.readInt()).filter(_ > 0)
    } { _ => print("""Level must be greater than 0 > """) }
  }
  ...
  private def player(board: Board, player: Player): AbstractPlayer = {
    ...
    retryUntilSuccess {
      Try(StdIn.readLine().toUpperCase() match {
        case "C" => new ComputerPlayer(board, player, maxLevel())
